...
fun JsonValue.toCompactJson(): String {
 ...
 * JSON.stringify, but only below 10^15: past 2^53 a Double cannot hold
 * exact integers anyway, so larger values keep Kotlin's exponent form.
 * Negative zero is excluded from the integer path because "-0.0" and
 * "0" parse back to Doubles with different bit patterns, and the
 * round-trip law compares with boxed equality that tells them apart.
 *
 * NaN and the infinities fail fast: JSON.stringify's alternative,
 * silently writing null, turns an arithmetic bug into corrupted data.
 */
 ...
 * control range, nothing else. Forward slash stays bare (escaping it
 * is only a courtesy for embedding JSON in HTML script tags), and
 * non-ASCII text passes through raw; encoders that force everything
 * to backslash-u form are catering to ASCII-only transports.
 */
